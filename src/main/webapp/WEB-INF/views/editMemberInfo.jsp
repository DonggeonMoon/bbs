...
<header style="float:right">
    <c:choose><c:when test="${sessionScope.memberDto != null }"><c:out value="${sessionScope.memberDto.memberId }"/> 님
        <a href="logout">로그아웃</a></c:when><c:otherwise><a href="login">로그인</a></c:otherwise></c:choose>
</header>
<h1>회원 정보 변경</h1>
    ...
    <form action="editMemberInfo" method="post">
        <div>
            ...
            <div style="display:inline-block;width:40%;text-align:left;">
                <input style="width:60%;" type="text" name="memberId" value="<c:out value='${memberInfo.memberId }' />"
                       readonly>
            </div>
        </div>
        <div>
        ...
        <div>
            ...
            <div style="display:inline-block;width:40%;text-align:left;">
                <input style="width:60%;" type="email" name="memberEmail"
                       value="<c:out value='${memberInfo.memberEmail }' />" required>
            </div>
        </div>
